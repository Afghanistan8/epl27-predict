...
# Arsenal vs Coventry City, matchday 1 — kickoff 2026-08-21T19:00:00Z.
KICKOFF_TS = 1787338800
...
def _finished_page() -> str:
    return "Premier League. Arsenal 2-1 Coventry City. Full time. Match finished."


def _postponed_page() -> str:
    return (
        "Premier League. Arsenal v Coventry City. POSTPONED. "
...
def _run(name, fn):
    ...
    try:
        with vm.activate():
            c = deploy_contract(CONTRACT, vm, "Arsenal", "Coventry City",
                                "2026-08-21", KICKOFF_TS)
            fn(vm, c)
